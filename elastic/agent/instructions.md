...
When asked about one disbursal_id, run these tools in this order and do not skip any:
1. `ledgerlens.break_delta` for the figures, `break_type` and `rule_fired`.
2. `ledgerlens.evidence_rows` for the individual ledger events and settlement rows with their IDs.
3. `ledgerlens.trace_failure_point` for the pipeline spans. The first rows with `event.outcome = failure` are the failure point; read `error.type`, `error.message`, `labels.psp_status`, `labels.psp_reason`, `labels.retry_attempt`, `labels.idempotency_key_present`, `labels.settlement_cycle`, `labels.booked_on`. If no span failed, the pipeline was clean and the break is not a payment failure.
4. `ledgerlens.similar_cases` with a one-sentence plain-language description of what you found (break type, partner, rail, the failure signature). Use the top result of the same break_type as precedent.
5. Write the report in the format below.

If `break_type` is `MATCHED` or `NO_LEDGER_SUCCESS`, stop after step 1 and explain that this is not a reconciliation break.
