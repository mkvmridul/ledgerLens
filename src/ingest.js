...
async function bulkLoad(name, file, chunk = 500) {
  ...
  for (let i = 0; i < lines.length; i += chunk) {
    ...
    for (let attempt = 1; ; attempt++) {
      ...
      console.log(`  retrying ${name} batch ${i / chunk + 1} in 20s (${firstErr?.type}: ${String(firstErr?.reason).slice(0, 120)})`);
