...
//   2. the Elastic Workflow that opens a case (elastic/workflows/ledgerlens-open-case.yaml)
//   3. the ES|QL tools (elastic/tools/*.esql + tools.json) and the workflow tool
...
async function ensureWorkflow() {
  const yaml = readFileSync(join(ROOT, "elastic", "workflows", `${WORKFLOW_ID}.yaml`), "utf8");
  let wf;
  try {
    wf = await kbn("POST", "/api/workflows/workflow", { id: WORKFLOW_ID, yaml });
    console.log(`2. Workflow: created ${wf.id} (valid=${wf.valid})`);
  } catch (e) {
    if (!exists(e)) throw e;
    wf = await kbn("PUT", `/api/workflows/workflow/${WORKFLOW_ID}`, { yaml, enabled: true });
    console.log(`2. Workflow: updated ${WORKFLOW_ID}`);
  }
  const check = await kbn("GET", `/api/workflows/workflow/${WORKFLOW_ID}`);
  if (check.valid === false) throw new Error(`workflow YAML invalid: ${JSON.stringify(check.validationErrors ?? check).slice(0, 800)}`);
  return check.id ?? WORKFLOW_ID;
}
...
async function ensureTools(workflowId) {
  ...
  // The action: an Elastic Workflow exposed as a tool. The agent may only call it after human approval.
  const openCase = {
    id: "ledgerlens.open_case",
    type: "workflow",
    description:
      "Open a case for a reconciliation break and write an audit record, by running the ledgerlens-open-case Elastic Workflow. ONLY call this after the user has explicitly approved in this conversation. Inputs: title, disbursal_id, break_type, severity (low|medium|high|critical), report (full report text), approved_by.",
    tags: TAGS,
    configuration: { workflow_id: workflowId, wait_for_completion: true },
  };
  try {
    console.log(`3. Tool: ${await upsertTool(openCase)} ledgerlens.open_case (workflow -> ${workflowId})`);
  } catch (e) {
    delete openCase.configuration.wait_for_completion;
    console.log(`3. Tool: ${await upsertTool(openCase)} ledgerlens.open_case (workflow -> ${workflowId}, without wait_for_completion)`);
  }
  ids.push(openCase.id);
  return ids;
}
...
export async function setupAgent() {
  ...
  const toolIds = await ensureTools(workflowId);
  ...
  return { inferenceId, workflowId, toolIds };
}
